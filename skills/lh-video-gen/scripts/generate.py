...
def generate_audio(dialogue, output_path, voice, rate, tts_command=None):
    ...
    if tts_command:
        ...
        # 尝试使用内置的 PowerShell TTS
        import subprocess
        ps_script = f"""
        Add-Type -AssemblyName System.Speech
        $speech = New-Object System.Speech.Synthesis.SpeechSynthesizer
        $speech.SetOutputToWaveFile('{output_path}')
...
def generate_slide(subtitle, visual, output_path, width, height, index):
    ...
    print(f"    运行 Chrome 截图命令...", end="", flush=True)
    ...
    print(f" 成功 ({output_abs.stat().st_size} 字节)")
    return output_path
